...
import { createNewReviewService, getAllReviewByProductIdService, ReplyReviewService, deleteReviewService, getAllOrdersByUser } from '../../services/userService';
...
function ReviewProduct(props) {
  ...
  let loadDataOrder = () => {
    if (userData?.id) {
        let fetchOrder = async () => {
            let order = await getAllOrdersByUser(userData?.id)
  ...
  function checkStatusOrder(dataOrder, statusString) { 
    let allStatusMatch = true;
    
        for (let order of dataOrder) {
            if (order.statusId !== statusString) {
                allStatusMatch = false;
            }
    
        }
        return { allStatusMatch };
  }
  const { allStatusMatch } = checkStatusOrder(DataOrder, 'S6');
    
    function getId(dataOrder) {
        return dataOrder.map(order => order.orderDetail.map(item => item.product?.id));
    }
    const idpro = getId(DataOrder);
    function checkId(prodId) {
        if(prodId == id){
            return true
        }
    } 
    const result = idpro.some(checkId);
    console.log(result,allStatusMatch)

  let handleSaveComment = async () => {
    if (allStatusMatch && result) {
                ...
                return (
                  <div key={index} className="review_item">
                    <div className="media">
                      ...
                      <div className="media-body">
                        ...
                        {inputValues.user && inputValues.user.roleId === 'R1' && (
                          <a style={{ cursor: 'pointer' }} onClick={() => handleOpenModal(item.id)} className="reply_btn">
                            Phản hồi
                          </a>
                        )}
                      </div>
                    ...
                    <div style={{ marginLeft: '88px' }}>
                      ...
                      {item.image && <img onClick={() => openPreviewImage(item.image)} className="img-cmt" src={item.image}></img>}
                      {item.childComment &&
                        ...
                        item.childComment.map((item, index) => {
                          return (
                            <div key={index} className="box-reply">
                              ...
                              <p>{item.content}</p>
                              {inputValues.user && inputValues.user.roleId === 'R1' && (
                                <button onClick={() => handleDeleteReply(item.id)} className="btn-delete-reply" type="button">
                                  Xóa
                                </button>
                              )}
                            </div>
